...
export const Card = ({ children, style, padding = 24, onClick, hoverable }) => (
...
export const TopBar = ({ onLogin, onRegister }) => (
  <header style={{ height: 72, background: '#fff', borderBottom: '1px solid #E4E8EF', display: 'flex', alignItems: 'center', padding: '0 40px', position: 'sticky', top: 0, zIndex: 10 }}>
    <a href="#" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }}>
      <img src={MARK} style={{ height: 36 }} alt="" />
      <span style={{ fontFamily: 'Manrope, Inter, sans-serif', fontWeight: 800, fontSize: 22, color: '#1A2332', letterSpacing: '-0.01em' }}>СПЕКТР</span>
    </a>
...
export const Sidebar = ({ active, onNav, user }) => {
  ...
  return (
    <aside style={{ width: 248, background: '#0F2D4A', color: '#fff', display: 'flex', flexDirection: 'column', padding: '20px 14px', minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px 20px', borderBottom: '1px solid rgba(255,255,255,.08)', marginBottom: 14 }}>
        <img src={MARK} style={{ width: 32, height: 32 }} alt="" />
        <span style={{ fontFamily: 'Manrope, Inter, sans-serif', fontWeight: 800, fontSize: 20, letterSpacing: '-0.01em' }}>СПЕКТР</span>
      </div>
      {items.map(it => {
      ...
      <div style={{ marginTop: 'auto', padding: '14px 12px', borderTop: '1px solid rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', gap: 12 }}>
