...
// Простой drag-and-drop для упорядочивания шагов сборки
...
const MechanismDetail = ({ mech, onBack }) => {
  // Перемешиваем шаги детерминированно
...
const MechanismsScreen = ({ onBack, onNav, onLogout }) => {
  ...
  // Фильтруем по профессии (electricians vs mechanics)
  const profession = user.position?.toLowerCase().includes('электр') ? 'electrician' : 'mechanic';
  const list = MECHANISMS.filter(m => m.profession === profession);
          ...
          <>
            <header className="s-main-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, gap: 16 }}>
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div className="s-main-header-sub" style={{ fontSize: 14, color: '#3A4657', marginTop: 4 }}>Узлы и механизмы для {profession === 'electrician' ? 'электромонтёров' : 'слесарей'} · {list.length} заданий</div>
              </div>
