...
// Упрощённый кабинет внешнего слушателя — курсы, уроки, экзамены
const StudentDashboardScreen = ({ onLogout, onNav }) => {
  const { user } = useAuth();
  if (!user) return null;
  const enrolled = COURSES.find(c => c.id === user.enrolledCourse);

  return (
    <div style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', background: '#F7F9FC', color: '#1A2332' }}>
      {/* Кастомный headerbar (зелёный для слушателя) */}
      ...
      <main style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 40px 60px' }}>
        ...
        <Alert tone="info" title={enrolled ? `Вы записаны на курс «${enrolled.title}»` : 'Вы пока не записаны на курс'} description={enrolled ? `Старт занятий: ${enrolled.nextStart} · Преподаватель: ${enrolled.teacher}` : 'Выберите курс из каталога Учебного центра.'} action={enrolled ? null : 'Каталог курсов'}/>
        ...
        <div className="s-card-grid-2" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 20, marginTop: 24, marginBottom: 24 }}>
          ...
          {enrolled && (
            <Card padding={28} style={{ borderLeft: '4px solid #1F7A3D' }}>
              ...
              <p style={{ fontSize: 14, color: '#475060', margin: '0 0 18px', lineHeight: 1.6 }}>{enrolled.direction} · {enrolled.duration} · {enrolled.format === 'mixed' ? 'смешанный формат' : enrolled.format === 'online' ? 'онлайн' : 'очно'}</p>
              ...
              <Button variant="success" iconRight="arrow">Перейти к занятиям</Button>
            </Card>
          )}
        ...
        <div className="s-action-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {[
            { ic: 'film',    t: 'Видео-занятия',   s: '8 уроков', bg: '#EEF3F8', fg: '#1B4B7A' },
            { ic: 'target',  t: 'Промежуточные тесты', s: '3 теста', bg: '#FDF4E7', fg: '#C77A0F' },
            { ic: 'graduation', t: 'Итоговый экзамен', s: 'после программы', bg: '#EAF5EE', fg: '#1F7A3D' },
          ].map((c, i) => (
            <Card key={i} padding={20} hoverable>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, background: c.bg, color: c.fg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
