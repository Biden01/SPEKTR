...
const AdminAuditScreen = ({ onNav, onLogout }) => {
  ...
  const types = [
    ...
    { id: 'question_added', l: 'Изменения базы вопросов' },
    { id: 'lesson_added', l: 'Изменения уроков' },
    { id: 'user_added', l: 'Изменения пользователей' },
  ];

  const filtered = EVENTS.filter(e => {
    if (type !== 'all' && e.type !== type) return false;
    if (search) {
      const u = EMPLOYEES.find(x => x.id === e.userId);
      const blob = `${u?.fullName || e.userId} ${e.text}`.toLowerCase();
              ...
              {filtered.map((ev, i) => {
                ...
                return (
                  <tr key={ev.id} style={{ background: i%2 ? '#F7F9FC' : '#fff' }}>
                    <td style={{ padding:'10px 16px', fontSize:13, fontFamily:'JetBrains Mono, monospace', color: '#5B6778' }}>{ev.dt}</td>
                    <td style={{ padding:'10px 16px', fontSize:13, fontWeight: 600 }}>{u?.fullName.split(' ').slice(0,2).join(' ') || ev.userId}</td>
                    ...
                    <td style={{ padding:'10px 16px', fontSize:12, fontFamily:'JetBrains Mono, monospace', color: '#5B6778' }}>10.0.{i % 256}.{(i * 7) % 256}</td>
                  </tr>
                );
              })}
            </tbody>
