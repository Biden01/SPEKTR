...
const AdminOverviewScreen = ({ onNav, onLogout }) => {
  ...
  const dailyToday = RESULTS.filter(r => r.date === '22.04.2026' && r.testType === 'daily').length;
  const overdueAnnual = EMPLOYEES.filter(e => e.testStatus === 'overdue_annual' || e.testStatus === 'failed_annual').length;
  const overdueMed = EMPLOYEES.filter(e => e.testStatus === 'overdue_medical').length;

  // Активность за 30 дней (рандомная для демо)
  const days = Array.from({ length: 30 }, (_, i) => ({
    d: i + 1,
    n: 8 + Math.round(Math.sin(i * 0.5) * 4 + Math.cos(i * 0.3) * 3 + i * 0.2),
  }));
  const maxN = Math.max(...days.map(d => d.n));

  // Распределение по категориям (% сдач)
  const catStats = CATEGORIES.map(c => ({
    ...c,
    avg: 70 + Math.round(Math.cos(c.id.charCodeAt(0)) * 15 + 10),
  }));

  // Требуют внимания
  const attention = EMPLOYEES.filter(e => e.testStatus !== 'ok').slice(0, 5);
            ...
            {catStats.map(c => (
              <div key={c.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
                  <span>{c.short}</span>
                  <span style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 600 }}>{c.avg}%</span>
                </div>
                <div style={{ height: 8, background: '#EEF1F6', borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ width: `${c.avg}%`, height: '100%', background: c.color, transition: 'width 700ms cubic-bezier(.2,0,0,1)' }}/>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card padding={0}>
          ...
          <div>
            {EVENTS.slice(0, 8).map((ev, i) => {
              ...
              const u = EMPLOYEES.find(x => x.id === ev.userId);
              const name = u?.fullName.split(' ').slice(0, 2).join(' ') || ev.userId;
              return (
                <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px', borderBottom: i === 7 ? 'none' : '1px solid #EEF1F6' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, fontSize: 13, color: '#1A2332' }}>
                    <strong>{name}</strong> {ev.text}
                  </div>
                  <div style={{ fontSize: 11, color: '#8A95A5', fontFamily: 'JetBrains Mono, monospace', flexShrink: 0 }}>{ev.dt.split(' ')[1]}</div>
                </div>
              );
            })}
          </div>
