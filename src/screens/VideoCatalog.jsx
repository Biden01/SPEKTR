...
const VideoCatalogScreen = ({ onOpenLesson, onBack, onNav, onLogout }) => {
  ...
  const statusChip = { done: { tone: 'ok', l: 'Просмотрено' }, todo: { tone: 'neutral', l: 'Не пройдено' }, new: { tone: 'info', l: 'Новый' } };
      ...
      <main className="s-main" style={{ flex: 1, padding: '28px 40px 60px', overflow: 'auto' }}>
        <header className="s-main-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, gap: 16 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div className="s-main-header-sub" style={{ fontSize: 14, color: '#3A4657', marginTop: 4 }}>{LESSONS.length} уроков · 5 направлений</div>
          </div>
        ...
        <div className="s-card-grid-2" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 24 }}>
          <aside>
            ...
            <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: '#5B6778', marginBottom: 10 }}>Статус</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {[{id:'all',l:'Все'},{id:'done',l:'Просмотрено'},{id:'todo',l:'Не пройдено'},{id:'new',l:'Новые'}].map(t => (
                <button key={t.id} type="button" onClick={() => setStatusFilter(t.id)}
                  style={{
                    padding: '8px 12px', border: 'none', background: statusFilter === t.id ? '#EEF3F8' : 'transparent',
                    color: statusFilter === t.id ? '#1B4B7A' : '#3A4657', fontWeight: statusFilter === t.id ? 600 : 500, fontSize: 14,
                    borderRadius: 6, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit',
                  }}>{t.l}</button>
              ))}
            </div>
                    ...
                    <Card key={l.id} padding={0} hoverable onClick={() => onOpenLesson && onOpenLesson(l)}>
                      ...
                      <div style={{ padding: 16 }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <Chip tone={statusChip[l.status].tone}>{statusChip[l.status].l}</Chip>
