...
const LoginScreen = ({ onEnter, onBack }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F9FC', fontFamily: 'Inter, sans-serif', color: '#1A2332', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
      {/* Left brand panel */}
      <div style={{ background: 'linear-gradient(160deg, #1B4B7A 0%, #0F2D4A 100%)', color: '#fff', padding: '48px 56px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onClick={onBack}>
          <img src={LOGO_WHITE} style={{ height: 40 }} alt=""/>
        </div>
        <div style={{ marginTop: 'auto', position: 'relative' }}>
          ...
          <p style={{ fontSize: 16, color: '#A8C0D6', lineHeight: 1.6, marginTop: 16, maxWidth: 440 }}>Используйте табельный номер для входа как сотрудник ВТС или зарегистрируйтесь во внешнем Учебном центре.</p>
        </div>
      </div>
      {/* Right form */}
